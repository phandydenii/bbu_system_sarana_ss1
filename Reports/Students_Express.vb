...
Public Class Students_Express

    Private Sub Students_Express_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            If ViewerFrm.cboDegree.SelectedValue Is Nothing Then
                ...
                If ViewerFrm.chkAllField.Checked = True Then
                    Dim cmd As New SqlCommand("SELECT * FROM V_ACADEMIC_OFFICE_REPORT_STUDENT_EXPRESS WHERE  DEGREE_ID=@degree_id AND SCHOOL_ID=@school_id  AND PROMOTION_ID=@promotion_id AND STAGE_ID=@stage_id AND GROUP_ID IN (" & groups & ") AND TERM_NO= " & Term.GetTermNo(Convert.ToInt32(ViewerFrm.txtYear.Text), Convert.ToInt32(ViewerFrm.txtSemester.Text)) & "  AND EXPRESS_DATE BETWEEN @from AND @to", DbInterface.Connection)
                    Dim da As New SqlDataAdapter(cmd)
                ...
                Else
                    Dim cmd As New SqlCommand("SELECT * FROM V_ACADEMIC_OFFICE_REPORT_STUDENT_EXPRESS WHERE DEGREE_ID=@degree_id AND SCHOOL_ID=@school_id AND FIELD_ID=@field_id AND PROMOTION_ID=@promotion_id AND STAGE_ID=@stage_id AND GROUP_ID IN (" & groups & ") AND TERM_NO= " & Term.GetTermNo(Convert.ToInt32(ViewerFrm.txtYear.Text), Convert.ToInt32(ViewerFrm.txtSemester.Text)) & "  AND EXPRESS_DATE BETWEEN @from AND @to", DbInterface.Connection)
                    Dim da As New SqlDataAdapter(cmd)
                    cmd.Parameters.Add("@degree_id", SqlDbType.Int).Value = CType(ViewerFrm.cboDegree.SelectedItem, Degree).DegreeId
                    cmd.Parameters.Add("@school_id", SqlDbType.Int).Value = CType(ViewerFrm.cboSchool.SelectedItem, School).SchoolId
